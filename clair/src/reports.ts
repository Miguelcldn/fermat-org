...
namespace Issues {
     ...
     */
    function report(stack: string, variables: any, env: string, message: string) {
        var req = new XMLHttpRequest(),
            url = this.helper.getAPIUrl("issues");
     ...
     * be created.
     */
    function assert(obj: any, expression: boolean, message: string) {
        'use strict';
        if (expression === false) {
            ...
            report(stack, variables, this.API_ENV, message);
        }
    }
}
